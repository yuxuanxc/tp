 ...
 * An itinerary is considered unique by comparing using {@code Itinerary#isSameItinerary(Itinerary)}. As such,
 * adding and updating of itineraries uses Itinerary#isSameItinerary(Itinerary) for equality so as to ensure that
 * the itinerary being added or updated is unique in terms of identity in the UniqueItineraryList. However, the
 * removal of an itinerary uses Itinerary#equals(Object) so as to ensure that the itinerary with exactly the same
...
public class UniqueItineraryList implements Iterable<Itinerary> {
     ...
     * {@code items} must not contain duplicate itineraries.
     */
